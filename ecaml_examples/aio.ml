...
type read =
  | Accept of (file_descr * sockaddr, ![io], unit) continuation
  | Recv of
      bytes * int * int * msg_flag list * (int, ![io], unit) continuation
type write =
  | Send of bytes * int * int * msg_flag list * (int, ![io], unit) continuation

type timeout =
  | Sleep of (unit, ![io], unit) continuation

type runnable =
  | Thread : ('a, ![io], unit) continuation * 'a -> runnable
